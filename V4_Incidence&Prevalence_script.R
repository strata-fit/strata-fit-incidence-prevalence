...
data_node<- data_node_2008



# Patients with both RF and anti-CCP missing
incomplete_serology <- data_node %>%
  select(pat_ID, RF_positivity, anti_CCP) %>%
  filter(is.na(RF_positivity) & is.na(anti_CCP)) %>%
  distinct()

# Save to CSV
write.csv(incomplete_serology, "incomplete_serology.csv", row.names = FALSE)
